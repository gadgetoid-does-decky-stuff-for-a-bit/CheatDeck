...
const Normal: VFC<{ appid: number }> = ({ appid }) => {
  ...
  useEffect(() => {
    const { unregister } = SteamClient.Apps.RegisterForAppDetails(
      ...
      (detail: AppDetails) => {
        ...
        if (
          optionsString.match("heroicgameslauncher") ||
          optionsString.match("Emulation")
        ) {
          setIsSteam(false);
        }
      },
  ...
  const handleBrowse = async () => {
    ...
    const filePickerRes = await Backend.openFilePicker(defaultDir, true, [
      "exe",
      "EXE",
    ]);
  ...
  const saveOptions = () => {
    if (isSteam) {
      ...
      Backend.sendNotice(
        "Warning: This is not a steam game! settings will not be saved.",
      );
    }
  };
  ...
  return (
    <Focusable style={{ display: "flex", flexDirection: "column" }}>
      ...
      />
      {showLang && (
        <Field label="Language code" padding="none" bottomSeparator="thick">
          <Focusable
            style={{
              boxShadow: "none",
              display: "flex",
